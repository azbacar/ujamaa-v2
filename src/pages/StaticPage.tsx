  ...
  const renderContent = (content: string) => {
    return content.split('\n').map((line, i) => {
      ...
      if (line.startsWith('- ')) {
        ...
        return <li key={i} className="text-muted-foreground ml-4 mb-1" dangerouslySetInnerHTML={{ __html: formatInline(text) }} />;
      }
      if (line.trim() === '') return <br key={i} />;
      return <p key={i} className="text-muted-foreground mb-2" dangerouslySetInnerHTML={{ __html: formatInline(line) }} />;
    });
  };
