...
export default function SocialShareButtons({ title, description, className = '' }: SocialShareButtonsProps) {
  ...
  const shareLinks = [
    ...
    },
    {
      ...
      color: 'bg-[#6C63FF] hover:bg-[#5a52e0] text-white',
      href: `https://wanzani.azzhy.com/share?url=${encodeURIComponent(shareUrl)}&title=${encodeURIComponent(title)}`,
    },
  ];
      ...
      {shareLinks.map((link) => (
        ...
        >
          <Button
            variant="ghost"
            size="sm"
            className={`h-8 w-8 p-0 rounded-full ${link.color}`}
          >
            <link.icon />
          </Button>
        </a>
